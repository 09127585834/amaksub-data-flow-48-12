...
const CableTransactionPinPage = () => {
  ...
  const processTransaction = async () => {
    ...
    try {
      ...
      // Call GSUBZ cable API
      const { data: cableResponse, error: cableError } = await supabase.functions.invoke('gsubz-cable', {
        body: {
          selectedNetwork,
          selectedPlan,
          smartCardNumber,
          customerName,
          user_id: user.id,
          phone_number: userPhoneData?.phone_number || smartCardNumber
        }
      });

      if (cableError) {
        console.error('Cable purchase error:', cableError);
        setIsProcessing(false);
        setShowError(true);
        return;
      }

      if (!cableResponse?.success) {
        console.error('Cable purchase failed:', cableResponse);
        setIsProcessing(false);
        if (cableResponse?.insufficientBalance) {
          setShowInsufficientBalance(true);
        } else {
          setShowError(true);
        }
      ...
      navigate('/cable/purchase-success', {
        state: {
          transaction: cableResponse.transaction,
          selectedNetwork,
          selectedPlan,
          smartCardNumber,
          customerName,
...
export default CableTransactionPinPage;